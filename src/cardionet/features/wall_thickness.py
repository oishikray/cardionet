...
def compute_frame_profile_and_sectors(
    ...
    *,
    ...
    """
    Compute ray-wise wall thickness and sector IDs for a single frame/slice.
    """
    ...
    if not np.isfinite(yc) or not np.isfinite(xc):
        return angles, wt, sector_ids, (yc, xc)
    ...
    for i, theta in enumerate(angles):
        ...
        _, _, th = thickness_from_transitions(samples)
        wt[i] = th
...
def compute_wt_matrix_and_sector_ids(
    ...
    *,
    ...
    for frame in range(t):
        label_slice = labels_4d[:, :, slice_index, frame]
        _, _, lv = get_masks(label_slice)
        lv_areas[frame] = np.sum(lv)

        angles, wt, frame_sector_ids, centroid = compute_frame_profile_and_sectors(
...
def choose_slice(
    ...
    *,
    ...
    """
    if labels.ndim != 4:
        raise ValueError(f"Expected labels shaped (H, W, S, T), got {labels.shape}")
